...
const countMatchingProfiles = `-- name: CountMatchingProfiles :one
SELECT COUNT(*) FROM account.profile
WHERE user_name=$1
`

func (q *Queries) CountMatchingProfiles(ctx context.Context, userName string) (int64, error) {
	row := q.db.QueryRowContext(ctx, countMatchingProfiles, userName)
	var count int64
	err := row.Scan(&count)
	return count, err
}
